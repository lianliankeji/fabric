...
func (t *EventSender) Invoke(stub shim.ChaincodeStubInterface, function string, args []string) (
	...
	if function == "account" {
		...
		Avalbytes, errC := stub.GetState(A)
		...
		if Avalbytes != nil {
			...
			//event
			stub.SetEvent("error", []byte("account have opened"))

			return nil, errors.New("account have opened")
		}
	...
	if function == "recharge" || function == "takeCash" {
		...
		if Avalbytes == nil {
			...
			//event
			//stub.SetEvent("evtsender", []byte("Entity not found"))
...
func (t *EventSender) delete(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {

	if len(args) != 1 {

		//event
		stub.SetEvent("error", []byte("Incorrect number of arguments. Expecting 1"))
	...
	if err != nil {
		...
		//event
		stub.SetEvent("error", []byte("Failed to delete state"))
